...
# ── KEEP ALIVE (server band nahi hoga) ────────────────────────────────────────
@app.route('/ping')
def ping():
    return "Veda is alive! 🌸", 200
...
- Agar koi image share kare — usse deeply analyze karo.
- Agar koi PDF share kare — uski content clearly summarize karo.
...
def save_user(uid, data):
    ...
    if REPLIT_DB:
        try: 
            replit_db[f"u_{uid}"] = json.dumps(data)
            return
        except: pass
...
def chat():
    try:
        ...
        reply = gemini_generate("gemini-1.5-flash", ctx, VEDA_PROMPT)
...
def chat_image():
    try:
        ...
        # REST API format corrected (inlineData and mimeType)
        contents = [{"role":"user","parts":[
            {"inlineData": {"mimeType": mime, "data": b64}},
            {"text": caption or "Is image mein kya hai? Poora detail mein batao."}
        ]}]
        
        reply = gemini_generate("gemini-1.5-flash", contents, VEDA_PROMPT, 0.7, 2048)
        
        user = get_user(uid)
        entry = {
            "user":f"[Image] {caption}",
            "veda":reply,
            "time":time.time(),
            "timestamp":datetime.now().strftime("%d %b %Y, %I:%M %p"),
            "mode":"vision"
...
def chat_pdf():
    try:
        ...
        if not PDF_AVAILABLE: 
            return jsonify({"reply":"PDF library nahi hai 🙏"})
...
def chat_search():
    try:
        ...
        reply = gemini_generate("gemini-1.5-flash", contents, VEDA_PROMPT, 0.7, 1024)
